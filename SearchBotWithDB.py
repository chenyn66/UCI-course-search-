...
def lambda_handler(event, context):
    ...
    table = dynamodb.Table('SearchBotReceiver')
    response = table.scan()['Items']
    BotList = []
    for BotArgs in response:
        BotList.append(SearchBot([BotArgs['Condition']['Dept'] ,BotArgs['Condition']['Num']],
                                 BotArgs['Condition']['In'],
                                 BotArgs['Condition']['Ex'],
                                 BotArgs['Condition']['Rstr'],
                                 (SENDER ,PASSWORD ,BotArgs['Email'])))
...
class SearchBot:
    ...
    def __init__(self, CourseArgs, IncludeList=None, ExclueList=None, RstrList=None,
                 MailInfo: ('sender', 'Password', 'receiver') = None,
                 gap=1):
    ...
    def _request(self):
        paralib = parse.parse_qs(
            'YearTerm=2017-92&ShowComments=on&ShowFinals=on&Breadth=ANY&Dept=I%26C+SCI&CourseNum=46&Division=ANY&CourseCodes=&InstrName=&CourseTitle=&ClassType=ALL&Units=&Days=&StartTime=&EndTime=&MaxCap=&FullCourses=ANY&FontSize=100&CancelledCourses=Exclude&Bldg=&Room=&Submit=Display+Web+Results',
            True)

        for key, value in paralib.items():
            paralib[key] = value[0]
        paralib['Dept'] = self._args[0]
        paralib['CourseNum'] = self._args[1]
        self._args = paralib
    ...
    def _analyse(self, msg):
        ...
        courses = []
        for line in msg:
            if line.lstrip().startswith('<tr valign="top">') or line.lstrip().startswith(
    ...
    def runOnetime(self):
        ...
        if self.ResultOnConsole:
            print("Begin to Check " + str(self._args) + " for " + self._Mail[2])
        while True:
            try:
